...
def softmax_cross_entropy_loss(Z, Y=np.array([])):
    ...
    '''
    A = ( np.exp( Z - np.amax(Z, axis=0) ) ) / np.sum(np.exp(Z - np.amax(Z, axis=0)), axis=0)    #or np.amax(Z, axis=0).reshape(1, Z.shape[1])
    cache = {}
    cache['Z'] = Z

    m = Y.shape[1]

    loss = (-1/m) * np.sum(np.log(np.choose(Y.astype(np.int64), A)))
...
def softmax_cross_entropy_loss_der(Y, cache):
    ...
    '''
    A = (np.exp(cache['Z'] - np.amax(cache['Z'], axis=0))) / np.sum(np.exp(cache['Z'] - np.amax(cache['Z'], axis=0)), axis=0)

    Y_one_hot = ( np.eye(cache['Z'].shape[0])[Y.astype(np.int64).reshape(-1)] ).T
...
def multi_layer_network(X, Y, net_dims, num_iterations=500, learning_rate=0.2, decay_rate=0.01):
    ...
    for ii in range(num_iterations):
        ...
        # A = {}
        # cache = {}
        # for layer in range(net_dims):
        #     A['A'+str(layer+1)] , cache['cache' + str(layer + 1)] = layer_forward(A0, parameters['W' + str(layer+1)] ,  parameters['b' + str(layer+1)], 'relu')

        AL, caches = multi_layer_forward(X, parameters)
        Asfmx, cache, cost = softmax_cross_entropy_loss(AL, Y)
...
def main():
    ...
    trAcc = 100 * np.sum(train_Pred == train_label) / train_label.shape[1]          #None
    teAcc = 100 * np.sum(test_Pred == test_label) / test_label.shape[1]             #None
    print("Accuracy for training set is {0:0.3f} %".format(trAcc))
    print("Accuracy for testing set is {0:0.3f} %".format(teAcc))
    
    ###plot costs vs iteration
    iteration = [iter for iter in range(0, 500, 10)]
    plt.plot(iteration, costs, color='blue')
    points = plt.scatter(iteration, costs, c='red', s=5)
    plt.title("Neural Networks - 3 Layers")
    plt.xlabel("Iteration")
    plt.ylabel("Train Error")
    plt.text(150, 1.6, "Training Accuracy = " + str(trAcc))
    plt.text(150, 1.4, "Testing  Accuracy = " + str(teAcc))
    plt.savefig("Neural Network3")
    plt.show()
